...
- `GET /log?limit=N` — the system-log card, newest first (limit clamped to 500). Events are appended as JSONL to `events.jsonl` beside `tracking.txt`, so the mount that keeps the settings keeps the history; `log_event()` never raises and is guarded by a lock, because the threaded dev server means the device and a browser can write at once. The file is trimmed to `LOG_MAX_ENTRIES` once it passes `LOG_TRIM_BYTES`. Events: `checkin` (ip, battery, asset, album, plus `mac`/`rssi` **only if the firmware sends `X-Device-Mac`/`X-Device-Rssi`** — HTTP carries no MAC and the container cannot read the LAN's ARP table), `sleep`, `settings_saved` (with a before/after diff), `config_reloaded`, `tracking_reset`, `error`, `startup`.
- `POST /notify/test` — sends one message now, so the credentials can be checked before the battery is actually low. The low-battery warning itself is triggered from `/download` (the only place a battery reading arrives) and sent **on a thread**, because the frame gives up after 50s and must not wait on Telegram or LINE. Credentials come from the environment (`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`, `LINE_CHANNEL_TOKEN`/`LINE_USER_ID`) for the same reason `IMMICH_API_KEY` does: the settings page has no authentication. Rate limiting lives in `state.notify['last_sent']`, in memory, so a restart allows one extra message through. LINE requires the Messaging API; LINE Notify was discontinued in 2025.
...
- `GET /preview` and `GET /preview/original` — back the settings page's "now showing" card. `/preview` returns the dithered 800x480 PNG captured as a side effect of the last `/download` (module global `last_photo`, memory only, `Cache-Control: no-store`). `/preview/original` proxies Immich's `/api/assets/{id}/thumbnail?size=preview`; it has to be proxied because the API key never reaches the browser, and thumbnails are asked for rather than originals because originals may be HEIC or RAW. Both 404 until the device has fetched an image at least once, and the card is omitted entirely in that state.
